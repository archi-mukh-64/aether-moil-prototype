...
export const ScenarioDiffIndicator = () => {
  ...
  return (
    <div className="panel-surface p-5 sm:p-6 border border-manganese-500/40 bg-gradient-to-r from-obsidian-950 via-obsidian-900 to-obsidian-950 font-mono text-xs shadow-2xl animate-fade-in space-y-4 rounded-2xl select-none">
      ...
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-obsidian-800">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-lg bg-obsidian-800 hover:bg-obsidian-700 text-zinc-300 hover:text-white text-[10px] font-bold transition-colors flex items-center gap-1 border border-obsidian-700"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div className="p-3 rounded-xl bg-obsidian-950/80 border border-obsidian-800 space-y-1">
          <div className="text-[10px] text-zinc-500 uppercase font-bold">{comm.production || 'Production'}</div>
          <div className="flex items-center gap-1.5 text-xs">
            <span className="text-zinc-400">{baselineYield.toLocaleString()}</span>
        ...
        <div className="p-3 rounded-xl bg-obsidian-950/80 border border-obsidian-800 space-y-1">
          <div className="text-[10px] text-zinc-500 uppercase font-bold">{lang === 'hi' ? 'कमी जोखिम' : lang === 'mr' ? 'तूट जोखीम' : 'Shortfall Risk'}</div>
          <div className="flex items-center gap-1.5 text-xs">
            <span className="text-zinc-400">{baselineRisk}</span>
        ...
        <div className="p-3 rounded-xl bg-obsidian-950/80 border border-obsidian-800 space-y-1">
          <div className="text-[10px] text-zinc-500 uppercase font-bold">{lang === 'hi' ? 'फ्लीट स्वास्थ्य' : lang === 'mr' ? 'फ्लीट आरोग्य' : 'Fleet Health'}</div>
          <div className="flex items-center gap-1.5 text-xs">
            <span className="text-zinc-400">{baselineFleet}</span>
          ...
          <div className="text-[9px] text-zinc-500">{lang === 'hi' ? 'ढुलाई ड्रैग प्रभाव' : lang === 'mr' ? 'वाहतूक ड्रॅग प्रभाव' : 'Haul Drag Effect'}</div>
        </div>

        {/* Metric 4: Sump Inflow */}
        <div className="p-3 rounded-xl bg-obsidian-950/80 border border-obsidian-800 space-y-1">
          <div className="text-[10px] text-zinc-500 uppercase font-bold">{lang === 'hi' ? 'सम्प अंतर्वाह' : lang === 'mr' ? 'सम्प प्रवाह' : 'Sump Inflow'}</div>
          <div className="flex items-center gap-1.5 text-xs">
            <span className="text-zinc-400">{baselineWater}</span>
        ...
        <div className="p-3 rounded-xl bg-obsidian-950/80 border border-obsidian-800 space-y-1">
          <div className="text-[10px] text-zinc-500 uppercase font-bold">{lang === 'hi' ? 'समग्र खतरा' : lang === 'mr' ? 'एकूण धोका' : 'Overall Threat'}</div>
          <div className="flex items-center gap-1.5 text-xs">
          ...
          <div className="text-[9px] text-zinc-500">{lang === 'hi' ? 'स्कोर:' : lang === 'mr' ? 'गुण:' : 'Score:'} {detectionScore}</div>
        </div>

        {/* Metric 6: Trust Score */}
        <div className="p-3 rounded-xl bg-obsidian-950/80 border border-obsidian-800 space-y-1">
          <div className="text-[10px] text-zinc-500 uppercase font-bold">{lang === 'hi' ? 'विश्वास अंशांकन' : lang === 'mr' ? 'विश्वास कॅलिब्रेशन' : 'Trust Calibration'}</div>
          <div className="flex items-center gap-1.5 text-xs">
            <span className="text-zinc-400">{baselineTrust}</span>
