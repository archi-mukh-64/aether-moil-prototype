...
export const SignalPanel = () => {
  ...
  if (!activeScenario) {
    return (
      <div className="panel-surface p-6 sm:p-8 border border-obsidian-750 font-mono text-xs text-center space-y-3 select-none">
        <Radio className="w-8 h-8 text-zinc-600 mx-auto animate-pulse" />
  ...
  return (
    <div className="panel-surface p-6 sm:p-8 border border-hazard-500/40 bg-gradient-to-b from-hazard-950/15 via-obsidian-900 to-obsidian-950 font-mono text-xs space-y-6 shadow-2xl animate-fade-in select-none">
      ...
      {causal.length > 0 && (
        <div className="p-4 rounded-xl bg-obsidian-950/90 border border-obsidian-800 space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1 font-mono text-xs">
            {causal.map((c, idx) => (
              <React.Fragment key={idx}>
                <div className="px-3 py-1.5 rounded-lg bg-obsidian-900 border border-obsidian-750 flex items-center gap-2">
                  <span className="text-zinc-400 text-[11px]">{c.step}:</span>
        ...
        {activeScenario.signals.map((sig, idx) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between pt-2 border-t border-obsidian-800 text-[10px] text-zinc-400">
                <span className="text-zinc-500">{lang === 'hi' ? 'विचलन:' : lang === 'mr' ? 'विचलन:' : 'Drift:'} <strong className="text-white">{sig.magnitude}</strong></span>
                <span>{lang === 'hi' ? 'ताजगी:' : lang === 'mr' ? 'ताजेपणा:' : 'Freshness:'} <strong className="text-zinc-300">{sig.freshness}</strong></span>
              </div>
            </div>
          );
        })}
