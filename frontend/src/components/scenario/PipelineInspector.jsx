...
export const PipelineInspector = () => {
  ...
  if (!activeScenario) {
    return (
      <div className="panel-surface p-8 border border-obsidian-750 font-mono text-xs text-center space-y-3">
        <Terminal className="w-8 h-8 text-zinc-600 mx-auto animate-pulse" />
  ...
  return (
    <div className="panel-surface p-6 sm:p-8 border border-telemetry-500/40 bg-gradient-to-b from-telemetry-950/15 via-obsidian-900 to-obsidian-950 font-mono text-xs space-y-6 shadow-2xl animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-obsidian-800 gap-2">
        ...
        <div className="px-2.5 py-1 rounded-lg bg-obsidian-950 border border-obsidian-800 text-[10px] text-zinc-400 font-bold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-3">
        ...
        <div className="p-4 rounded-xl bg-obsidian-950/80 border border-obsidian-800 space-y-2">
          <div className="flex items-center justify-between text-[10px] text-zinc-500 font-bold pb-1 border-b border-obsidian-850">
            <span>01 // INPUTS</span>
          ...
          <div className="space-y-1 text-[11px]">
            ...
            <div>Window: <strong className="text-zinc-300">{scenarioTimeHorizon || '24 HOURS'}</strong></div>
            <div>Target: <strong className="text-zinc-300">{activeMine?.dailyTarget?.toLocaleString() || 6200} TPD</strong></div>
          </div>
        </div>
            ...
            {signals.slice(0, 2).map((s, idx) => {
              ...
              return (
                <div key={idx} className="truncate">
                  <span className="text-zinc-500">{sigName.split(' ')[0]}:</span> <strong className="text-zinc-200">{s.value || 'Nominal'}</strong>
                </div>
              );
            })}
      ...
      <div className="p-4 rounded-xl bg-obsidian-950/90 border border-obsidian-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
        ...
        <span className="text-zinc-500 text-[11px]">Calibrated with Mine Operational Constraints</span>
      </div>
