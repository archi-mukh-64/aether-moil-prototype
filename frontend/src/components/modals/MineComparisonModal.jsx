    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#F0EBE2]/85 backdrop-blur-xl animate-fade-in font-mono text-xs select-none">
      ...
      >
        ...
        <div className="p-4 bg-[#080b10] border-b border-[#141b27] flex flex-wrap items-center gap-2">
          <span className="text-[11px] text-[#5F625C] font-bold uppercase mr-2">
          ...
          <table className="w-full text-left border-collapse font-mono text-xs">
            <thead>
              <tr className="border-b border-[#1f293d]">
                <th className="pb-3 text-[#85877E] uppercase text-[10px]">
                ...
                {comparedMines.map((m) => (
                  <th key={m.id} className="pb-3 text-white font-bold text-xs">
                    <div>{m.name}</div>
                    <div className="text-[10px] text-[#85877E] font-normal">{m.district}, {m.state}</div>
                  </th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-[#141b27]">
              ...
              <tr>
                <td className="py-3 text-[#5F625C]">{lang === 'hi' ? 'खदान प्रकार' : lang === 'mr' ? 'खाण प्रकार' : 'Mine Type'}</td>
                {comparedMines.map((m) => (
                  <td key={m.id} className="py-3 text-[#272A27] font-semibold">{m.mineType}</td>
                ))}
              </tr>

              <tr>
                <td className="py-3 text-[#5F625C]">{lang === 'hi' ? 'दैनिक लक्ष्य (TPD)' : lang === 'mr' ? 'दैनिक उद्दिष्ट (TPD)' : 'Daily Target (TPD)'}</td>
                {comparedMines.map((m) => (
              ...
              <tr>
                <td className="py-3 text-[#5F625C]">{lang === 'hi' ? 'औसत अयस्क ग्रेड' : lang === 'mr' ? 'सरासरी खनिज प्रत' : 'Average Ore Grade'}</td>
                {comparedMines.map((m) => (
              ...
              <tr>
                <td className="py-3 text-[#5F625C]">{lang === 'hi' ? 'रिकवरी दर' : lang === 'mr' ? 'पुनर्प्राप्ती दर' : 'Recovery Rate'}</td>
                {comparedMines.map((m) => (
              ...
              <tr>
                <td className="py-3 text-[#5F625C]">{lang === 'hi' ? 'एचईएमएम फ्लीट इकाइयां' : lang === 'mr' ? 'एचईएमएम फ्लीट युनिट्स' : 'HEMM Fleet Units'}</td>
                {comparedMines.map((m) => (
                  <td key={m.id} className="py-3 text-[#272A27]">{m.fleetCount} Units</td>
                ))}
              </tr>

              <tr>
                <td className="py-3 text-[#5F625C]">{lang === 'hi' ? 'ऐतिहासिक कमी प्रवृत्ति' : lang === 'mr' ? 'ऐतिहासिक तूट प्रवृत्ती' : 'Historical Shortfall Tendency'}</td>
                {comparedMines.map((m) => (
                  <td key={m.id} className="py-3 text-[#272A27]">{Math.round((m.historicalShortfallTendency || 0.12) * 100)}%</td>
                ))}
              </tr>

              <tr>
                <td className="py-3 text-[#5F625C]">{lang === 'hi' ? 'यूएनएफसी रिजर्व क्षमता' : lang === 'mr' ? 'यूएनएफसी साठा संभाव्यता' : 'UNFC Reserve Potential'}</td>
                {comparedMines.map((m) => (
              ...
              <tr>
                <td className="py-3 text-[#5F625C]">{lang === 'hi' ? 'एआई ट्रस्ट स्कोर' : lang === 'mr' ? 'एआय विश्वास निर्देशांक' : 'AI Trust Score'}</td>
                {comparedMines.map((m) => (
