...
export const ForecastPreview = ({ externalScenarioId, onScenarioSelect }) => {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      if (!p) return null;
      return (
        <div className="p-4 rounded-xl bg-obsidian-950/95 border border-obsidian-700 shadow-2xl font-mono text-xs text-zinc-200 select-none min-w-[280px] space-y-2.5">
          <div className="font-bold text-white pb-1.5 border-b border-obsidian-800 flex items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              <span className="text-manganese-400 font-bold">{p.day_label}</span>
              <span className="text-zinc-400 font-normal">({p.date})</span>
            </div>
            <span className="text-[10px] px-2 py-0.5 rounded bg-obsidian-850 text-manganese-300 font-bold">{mine_name}</span>
          </div>
          ...
          <div className="space-y-1.5 font-mono">
            ...
            {p.shortfall_tpd > 0 && (
              <div className="flex justify-between gap-4 text-hazard-400 font-bold pt-1.5 border-t border-obsidian-800">
            ...
            <div className="pt-1.5 text-[10px] text-zinc-400 flex flex-col gap-0.5 border-t border-obsidian-855">
              <div className="text-zinc-500 uppercase">{lang === 'hi' ? 'घटना स्थिति / चालक:' : lang === 'mr' ? 'घटना स्थिती / घटक:' : 'Event Marker / Main Driver:'}</div>
              <strong className="text-amber-300">{p.event_marker}</strong>
              <span className="text-zinc-300">{p.main_driver}</span>
            </div>
          </div>
        </div>
      );
    ...
    <section className="space-y-8 font-mono select-none">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-obsidian-800 pb-6">
        <div>
          ...
          <p className="text-xs sm:text-sm text-zinc-400 mt-1 max-w-3xl font-mono">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold transition-all border ${
              showDriverAnalysis
                ...
                : 'bg-obsidian-900 text-zinc-400 border-obsidian-750'
            }`}
          >
      ...
      {mineTransitionDelta && (
        <div className="p-4 rounded-2xl bg-gradient-to-r from-[#0c1f24] via-obsidian-900 to-[#0c1f24] border border-cyan-500/40 shadow-lg font-mono text-xs text-zinc-200">
          <div className="flex items-center justify-between gap-4 pb-2 border-b border-obsidian-800">
            ...
            <div className="text-[11px] text-zinc-400 font-bold">
              {mineTransitionDelta.fromMine} ({mineTransitionDelta.fromTarget.toLocaleString()} TPD) → <span className="text-white">{mineTransitionDelta.toMine} ({mineTransitionDelta.toTarget.toLocaleString()} TPD)</span>
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mt-3">
            <div className="p-2 rounded-xl bg-obsidian-950 border border-obsidian-800">
              <div className="text-[10px] text-zinc-400">Target Delta</div>
            ...
            <div className="p-2 rounded-xl bg-obsidian-950 border border-obsidian-800">
              <div className="text-[10px] text-zinc-400">14-Day Mean Yield</div>
            ...
            <div className="p-2 rounded-xl bg-obsidian-950 border border-obsidian-800">
              <div className="text-[10px] text-zinc-400">Shortfall Volume Delta</div>
            ...
            <div className="p-2 rounded-xl bg-obsidian-950 border border-obsidian-800">
              <div className="text-[10px] text-zinc-400">Risk Profile Shift</div>
              <div className="text-sm font-bold text-amber-300 mt-0.5">
                {mineTransitionDelta.fromRisk} → {mineTransitionDelta.toRisk}
              </div>
            </div>
            <div className="p-2 rounded-xl bg-obsidian-950 border border-obsidian-800">
              <div className="text-[10px] text-zinc-400">Peak Shock Day</div>
              <div className="text-sm font-bold text-hazard-400 mt-0.5">
                {mineTransitionDelta.peakDay}
              </div>
            </div>
            <div className="p-2 rounded-xl bg-obsidian-950 border border-obsidian-800">
              <div className="text-[10px] text-zinc-400">Stabilization Day</div>
      ...
      {showDriverAnalysis && (
        <div className="p-5 rounded-2xl bg-gradient-to-r from-obsidian-950 via-obsidian-900 to-obsidian-950 border border-cyan-500/40 shadow-xl font-mono text-xs text-zinc-200">
          <div className="flex flex-wrap items-center justify-between gap-4 pb-3 border-b border-obsidian-800">
            ...
            <div className="flex flex-wrap items-center gap-3 text-[11px]">
              <div className="px-3 py-1.5 rounded-xl bg-obsidian-900 border border-obsidian-750">
                <span className="text-zinc-400">Baseline Target: </span>
                <strong className="text-white">{targetTpd.toLocaleString()} TPD</strong>
              </div>
              <div className="px-3 py-1.5 rounded-xl bg-obsidian-900 border border-obsidian-750">
                <span className="text-zinc-400">14-Day Expected Mean: </span>
                <strong className="text-manganese-300">{kpis.avg_predicted_yield ? kpis.avg_predicted_yield.toLocaleString() : '0'} TPD</strong>
              </div>
              <div className="px-3 py-1.5 rounded-xl bg-obsidian-900 border border-obsidian-750">
                <span className="text-zinc-400">Projected Shortfall: </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
            {/* Primary Driver */}
            <div className="p-3.5 rounded-xl bg-obsidian-900/90 border border-obsidian-750 space-y-1.5">
      ...
      {showDiagnosticHud && (
        <div className="p-5 rounded-2xl bg-gradient-to-r from-obsidian-950 via-obsidian-900 to-obsidian-950 border border-amber-500/40 shadow-xl font-mono text-xs text-zinc-200">
          <div className="flex items-center justify-between pb-3 border-b border-obsidian-800">
            ...
            <div className="text-[11px] text-zinc-400">
              ID: <strong className="text-white">{mine_id}</strong> | Type: <strong className="text-manganese-300">{mine_type}</strong>
            </div>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 mt-4">
            <div className="p-2.5 rounded-xl bg-obsidian-900 border border-obsidian-750">
            ...
            <div className="p-2.5 rounded-xl bg-obsidian-900 border border-obsidian-750">
              <div className="text-[10px] text-zinc-400">D1 Logged Actual</div>
              <div className="text-sm font-bold text-cyan-400 mt-0.5">{historical_actual_d1.toLocaleString()} TPD</div>
            </div>
            <div className="p-2.5 rounded-xl bg-obsidian-900 border border-obsidian-750">
              <div className="text-[10px] text-zinc-400">Rain Sensitivity</div>
              <div className="text-sm font-bold text-amber-400 mt-0.5">{physical_profile.rainSens}x</div>
            </div>
            <div className="p-2.5 rounded-xl bg-obsidian-900 border border-obsidian-750">
              <div className="text-[10px] text-zinc-400">Haul Distance</div>
              <div className="text-sm font-bold text-zinc-200 mt-0.5">{physical_profile.haulDistKm} km</div>
            </div>
            <div className="p-2.5 rounded-xl bg-obsidian-900 border border-obsidian-750">
              <div className="text-[10px] text-zinc-400">Max Drainage</div>
              <div className="text-sm font-bold text-sky-400 mt-0.5">{physical_profile.maxDrainageM3h} m³/h</div>
            </div>
            <div className="p-2.5 rounded-xl bg-obsidian-900 border border-obsidian-750">
              <div className="text-[10px] text-zinc-400">Crusher Health</div>
              <div className="text-sm font-bold text-emerald-400 mt-0.5">{physical_profile.crusherHealthBase}%</div>
            </div>
            <div className="p-2.5 rounded-xl bg-obsidian-900 border border-obsidian-750">
              <div className="text-[10px] text-zinc-400">Surge Stockpile</div>
              <div className="text-sm font-bold text-purple-400 mt-0.5">{physical_profile.stockpileBufferT} T</div>
            </div>
            <div className="p-2.5 rounded-xl bg-obsidian-900 border border-obsidian-750">
              <div className="text-[10px] text-zinc-400">Protocol Code</div>
              <div className="text-sm font-bold text-hazard-400 mt-0.5">{physical_profile.protocolCode}</div>
            </div>
          </div>

          <div className="mt-3 pt-3 border-t border-obsidian-800 text-[11px] text-zinc-400 flex flex-wrap items-center justify-between gap-2">
            <span>Primary Infrastructure: <strong className="text-white">{physical_profile.keyInfrastructure}</strong></span>
          ...
          return (
            <button
              ...
              className={`p-4 rounded-2xl text-left transition-all border relative overflow-hidden flex flex-col justify-between ${
                isSelected
                  ? 'bg-obsidian-900/90 border-manganese-500 shadow-lg shadow-manganese-500/10'
                  : 'bg-obsidian-950/60 border-obsidian-800 hover:border-obsidian-700 hover:bg-obsidian-900/40'
              }`}
            >
              {isSelected && (
              ...
              <div>
                <div className="flex items-center justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="font-bold text-sm text-white">{scen.name}</span>
                  </div>
                  {isSelected && (
              ...
              <div className="mt-4 pt-3 border-t border-obsidian-800 flex items-center justify-between text-[10px] text-zinc-500 font-mono">
      ...
      {showWhatIfControls && (
        <div className="p-6 rounded-2xl bg-obsidian-950/80 border border-obsidian-800 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-obsidian-800">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-obsidian-900 border border-obsidian-750 text-xs text-zinc-300 hover:text-white flex items-center gap-2 self-start sm:self-auto transition-colors"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-xs font-mono">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full accent-sky-400 bg-obsidian-900 rounded-lg cursor-pointer"
              />
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full accent-amber-400 bg-obsidian-900 rounded-lg cursor-pointer"
              />
              <div className="flex justify-between text-[10px] text-zinc-500">
                <span>10% (Seized)</span>
                <span>50%</span>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full accent-manganese-400 bg-obsidian-900 rounded-lg cursor-pointer"
              />
              <div className="flex justify-between text-[10px] text-zinc-500">
                <span>20%</span>
                <span>60%</span>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full accent-cyan-400 bg-obsidian-900 rounded-lg cursor-pointer"
              />
              <div className="flex justify-between text-[10px] text-zinc-500">
                <span>20 m³/h</span>
                <span>Normal</span>
      ...
      <div className="p-6 rounded-2xl bg-obsidian-950/80 border border-obsidian-800 space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-obsidian-800">
          <div>
            <h3 className="font-bold text-white text-base sm:text-lg flex items-center gap-2">
              ...
              <span className="text-xs px-2 py-0.5 rounded bg-obsidian-900 border border-obsidian-750 text-manganese-300">
                {activePreset.name}
              </span>
            </h3>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs font-mono">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-zinc-300">Logged Actual (D1)</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="w-3 h-0.5 bg-manganese-400" />
              <span className="text-zinc-300">Predicted Yield</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="w-3 h-0.5 bg-red-400 border-dashed" />
              <span className="text-zinc-300">Target Quota</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="w-3 h-2 rounded bg-cyan-500/20 border border-cyan-500/40" />
              <span className="text-zinc-300">95% CI Band</span>
            </div>
            {showRainfall && (
              <div className="flex items-center gap-1.5">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 20, right: 30, left: 10, bottom: 20 }}>
              ...
              <Line 
                ...
                strokeWidth={3} 
                dot={{ r: 4, fill: activePreset.color, stroke: '#09090b', strokeWidth: 2 }}
            ...
            {forecast_points.slice(0, 14).map((pt) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-[9px] text-zinc-400 truncate mt-1 pt-1 border-t border-obsidian-800" title={pt.main_driver}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div className="p-4 rounded-2xl bg-obsidian-950/80 border border-obsidian-800">
          ...
          <div className="text-lg sm:text-2xl font-extrabold text-white mt-1 font-display">
            {kpis.avg_predicted_yield ? kpis.avg_predicted_yield.toLocaleString() : '0'} <span className="text-xs font-mono font-normal text-zinc-400">TPD</span>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-obsidian-950/80 border border-obsidian-800">
          ...
          <div className="text-lg sm:text-2xl font-extrabold text-hazard-400 mt-1 font-display">
            {kpis.total_shortfall_tonnes > 0 ? `-${kpis.total_shortfall_tonnes.toLocaleString()}` : '0'} <span className="text-xs font-mono font-normal text-zinc-400">T</span>
          </div>
          <div className="text-[11px] text-zinc-400 mt-1 font-mono">
        ...
        <div className="p-4 rounded-2xl bg-obsidian-950/80 border border-obsidian-800">
          ...
          <div className="text-lg sm:text-2xl font-extrabold text-emerald-400 mt-1 font-display">
            ₹{kpis.financial_exposure_cr || 0} <span className="text-xs font-mono font-normal text-zinc-400">Cr</span>
          </div>
          <div className="text-[11px] text-zinc-400 mt-1 font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 p-6 rounded-2xl bg-obsidian-950/80 border border-obsidian-800 flex flex-col justify-between space-y-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-zinc-300 mt-3 leading-relaxed font-sans">
              {ai_explanation}
            </p>
          </div>

          <div className="pt-4 border-t border-obsidian-800 flex flex-wrap items-center justify-between gap-3 text-xs font-mono text-zinc-400">
            <div className="flex items-center gap-2">
              <span className="text-zinc-500">{lang === 'hi' ? 'मॉडल स्थिति:' : lang === 'mr' ? 'मॉडेल स्थिती:' : 'Status:'}</span>
              <strong className="text-emerald-400">{models_status.status || 'ONLINE'} ({models_status.engine || 'AETHER-ENGINE'})</strong>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-zinc-500">{lang === 'hi' ? 'अयस्क ग्रेड:' : lang === 'mr' ? 'खनिज ग्रेड:' : 'Grade:'}</span>
        ...
        <div className="p-6 rounded-2xl bg-obsidian-950/80 border border-obsidian-800 space-y-4 font-mono">
          <div className="flex items-center justify-between border-b border-obsidian-800 pb-3">
            ...
            <span className="text-[10px] text-zinc-400">14D Impact</span>
          </div>
          ...
          <div className="space-y-3">
            {waterfall_drivers.map((drv, idx) => (
              <div 
                ...
                className="p-3 rounded-xl bg-obsidian-900 border border-obsidian-750 hover:border-obsidian-600 transition-all cursor-pointer space-y-1.5"
              >
                ...
                <div className="flex justify-between items-center text-[10px] text-zinc-400">
                  ...
                  <span className="text-zinc-500">{drv.current_val}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {selectedDriverDetail && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="max-w-md w-full p-6 rounded-2xl bg-obsidian-950 border border-obsidian-700 shadow-2xl font-mono space-y-4">
            <div className="flex justify-between items-center border-b border-obsidian-800 pb-3">
              ...
              <button 
                ...
                className="p-1 rounded-lg bg-obsidian-900 text-zinc-400 hover:text-white"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="space-y-2 text-xs text-zinc-300">
              <div className="flex justify-between py-1 border-b border-obsidian-900">
                ...
                <strong className="text-white">{selectedDriverDetail.category}</strong>
              </div>
              <div className="flex justify-between py-1 border-b border-obsidian-900">
                <span className="text-zinc-400">14-Day Mean Impact:</span>
                <strong className={selectedDriverDetail.impact_tpd < 0 ? 'text-hazard-400' : 'text-emerald-400'}>
                  {selectedDriverDetail.impact_tpd} TPD
                </strong>
              </div>
              <div className="flex justify-between py-1 border-b border-obsidian-900">
                <span className="text-zinc-400">Telemetry Value:</span>
                <strong className="text-cyan-300">{selectedDriverDetail.current_val}</strong>
              </div>
              <div className="flex justify-between py-1 border-b border-obsidian-900">
                <span className="text-zinc-400">Attribution Confidence:</span>
                <strong className="text-emerald-400">{selectedDriverDetail.confidence_pct}%</strong>
              </div>
            </div>

            <div className="p-3 rounded-xl bg-obsidian-900 border border-obsidian-800 space-y-1">
              <div className="text-[10px] text-zinc-400 uppercase font-bold">Algorithmic Recommendation:</div>
